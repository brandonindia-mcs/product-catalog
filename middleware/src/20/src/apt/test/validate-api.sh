...
#   VITE_APT_URL="http://localhost:32000/api" ./validate-api.sh
#
#   # External with explicit Host header (when using node-ip + nodePort)
#   HOST_HEADER="localhost" VITE_APT_URL="http://<node-ip>:32000/api" ./validate-api.sh
#
#   # In-cluster test (run inside a pod)
#   IN_CLUSTER=true VITE_APT_URL="http://product-catalog:2000/api" ./validate-api.sh
set -euo pipefail

: "${VITE_APT_URL:=http://localhost:32000/api}"
: "${TIMEOUT:=8}"
: "${HOST_HEADER:=}"
: "${IN_CLUSTER:=false}"
...
echo
echo "Validating API: ${VITE_APT_URL}"
...
# Primary checks
simple_get "${VITE_APT_URL%/}/"         # root
simple_get "${VITE_APT_URL%/}/health"   # common health
simple_get "${VITE_APT_URL%/}/api"      # api root
simple_get "${VITE_APT_URL%/}/api/health"

# POST to /api
simple_post "${VITE_APT_URL%/}/api"

# Optional in-cluster direct check hint
if [ "${IN_CLUSTER}" = "true" ]; then
  echo "Note: running in-cluster check. If the service uses containerPort 2000 you may need to target that port directly."
  echo
fi
